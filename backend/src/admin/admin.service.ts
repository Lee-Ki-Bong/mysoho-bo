...
export class AdminService {
  ...
  async getDashboardStats() {
    ...
    const [totalAdmins, newAdminsToday] = await Promise.all([
      ...
      this.adminRepository.count({ where: { created_at: Raw(alias => `${alias} >= :date`, { date: today }) } }),
    ]);
  ...
  async findAll(getAdminsDto: GetAdminsDto) {
    ...
    if (keyword) {
      query.where(
        new Brackets(qb => {
          qb.where('admin.adm_id LIKE :keyword', { keyword: `%${keyword}%` })
            .orWhere('admin.adm_name LIKE :keyword', { keyword: `%${keyword}%` })
            .orWhere('admin.adm_email LIKE :keyword', { keyword: `%${keyword}%` })
            .orWhere('adminSns.as_id LIKE :keyword', { keyword: `%${keyword}%` });
        }),
      );
    }
